...
export default function AppMenu({ history, location }) {
  ...
  const activeItem = findIndex(navLinks, ({ to }) => to === pathname);

  const navigationItems = navLinks.map(({ to, label }) => ({
    children: label, onClick: _ => history.push(to)
  }))
